...
class AttentionDataset(Dataset):
    def __init__(self, mentions, kb, tokenizer, args, istest, isdev=False):
        ...
        self.cand_num_train = args.cand_num_train
        ...
        self.count_prime = 0
        self.count_prime_unable = 0
        if args.train_kb_prime:
            with open(args.train_kb_prime) as f:
                self.prime_kb = json.load(f)
        else:
            self.prime_kb = {}
        self.args = args
        
        self.use_title_during_testing = args.use_title_during_testing
        self.fine_tune_setting = args.fine_tune_setting
    ...
    def train_dataset(self, index):
        ...
        # Rasel 
        name_lower = self.kb[kb_id]['title'].lower()
        ...
        candidates = random.sample(candidates, self.cand_num_train)
        ...
        if kb_id not in candidates:
            candidates = candidates[:self.cand_num_train - 1] + [kb_id]
        if len(candidates) < self.cand_num_train:
            sim_neg = random.sample(list(self.all_candidates), k=self.cand_num_train - len(candidates))
            candidates += sim_neg

        assert len(candidates) == self.cand_num_train
        ...
        for i in range(len(labels)):
            ...
            # RASEL : added title
            model_dir = self.args.model.replace(self.args.model.split('/')[-1], '')

            hist_file = f'{model_dir}prime_history.json'

            if os.path.exists(hist_file):
                with open(hist_file) as f:
                    prime_usage_hist = json.load(f)
            else:
                prime_usage_hist = {}


            if self.fine_tune_setting == 'prime':
                if is_ho:
                    if entity_id==kb_id and self.isdev==False and entity_id in self.prime_kb:
                        title_prime = self.prime_kb[entity_id]['title']
                        entity_title_text = remove_punctuation(title_prime)
                        self.count_prime +=1
                        if entity_id in prime_usage_hist:
                            prime_usage_hist[entity_id]['count']+=1
                        else:
                            prime_usage_hist[entity_id]={
                                'count':1,
                                'prime':entity_title_text,
                                'original':entity_names["title"]}
                        # print(f'count entity prime : {self.count_prime}')
                    elif entity_id!=kb_id:
                        entity_title_text = remove_punctuation(entity_names["title"])

                    elif entity_id not in self.prime_kb:
                        entity_title_text = remove_punctuation(entity_names["title"])
                        self.count_prime_unable +=1
                else:
                    entity_title_text = remove_punctuation(entity_names["title"])


                entity_title_text = entity_title_text + ' ' +entity_text
                entity_text_tokens = self.tokenizer.tokenize(entity_title_text)

                
            elif self.fine_tune_setting == 'original_title':
                entity_title_text = remove_punctuation(entity_names["title"])
                entity_title_text = entity_title_text + ' ' +entity_text
                entity_text_tokens = self.tokenizer.tokenize(entity_title_text)
            elif self.fine_tune_setting == 'only_definition':
                entity_text_tokens = self.tokenizer.tokenize(entity_text)
                

            # with open(hist_file, 'w') as f:
            #     json.dump(prime_usage_hist, f, indent=1)
        ...
        return self.count_prime_unable, self.count_prime, text_input_ids, text_attention_mask, candidates_input_ids, candidates_attention_masks, \
               mention_pos, labels
    ...
    def eval_dataset(self, index):
        ...
        for i in range(len(labels)):
            ...
            ## RASEL : added title
            if self.use_title_during_testing:
                entity_title_text = remove_punctuation(entity_names["title"])
                entity_text = entity_title_text + ' ' +entity_text
        ...
        mention_pos = torch.tensor(mention_pos).long()
        labels = torch.tensor(labels).view(-1).float()
        return data, text_input_ids, text_attention_mask, candiates_input_ids, candidates_attention_masks, \
               mention_pos, labels
...
def load_data(data_path):
    ...
    cleaned = []
    for i in data:
        if '[\E1]' in i['text'] and '\\E1]' in i['text']:
            i['text'] = i['text'].replace('.[\E1]', ' [\E1]').replace('[\E1].', ' [\E1] ')
            i['text'] = i['text'].replace(',[\E1]', ' [\E1]').replace('[\E1],', ' [\E1] ')
            i['text'] = i['text'].replace('.[\\E1]', ' [\\E1]').replace('[\\E1].', ' [\\E1] ')
            i['text'] = i['text'].replace(',[\\E1]', ' [\\E1]').replace('[\\E1],', ' [\\E1] ')
            i['text'] = i['text'].strip()
...
def get_attention_mention_loader(samples, kb, tokenizer, shuffle, is_test, args, is_dev=False):
    samples_set = AttentionDataset(samples, kb, tokenizer, args, is_test, is_dev)
    
    if is_test:
        # return make_single_loader(samples_set, 1, False)
        # rasel
        return make_single_loader(samples_set, args.batch, False)
    
    dataset = make_single_loader(samples_set, args.batch, shuffle)

    print(f'count_prime : {samples_set.count_prime}')
    return dataset
