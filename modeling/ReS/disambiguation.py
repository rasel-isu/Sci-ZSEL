...
class ExtractInfoEncoder(nn.Module):
    def __init__(self, pretrained_model, device, args):
        ...
        # # # rasel : load saved model
        # if args.saved_pt_model:
        #     model = RobertaModel(self.bert_config).from_pretrained(pretrained_model)
        #     state_dict = torch.load(args.saved_pt_model)
        #     state_dict = state_dict['sd'] 
        #     model.load_state_dict(state_dict, strict=False)
        #     print(f"Loaded saved {args.saved_pt_model} model sd!")
    ...
    def extract_text_info(self, input_ids, attention_mask, token_positions):
        last_hidden_states = self.model(input_ids=input_ids, attention_mask=attention_mask)[0]
