...
def get_gt_score(scores, label_ids):
    scores_all, indicies_all = scores.topk(scores.shape[1])
    scores_all = scores_all.tolist()
    indicies_all = indicies_all.tolist()
    label_ids_all = label_ids.tolist()
    gt_scores = []
    for s, i, l in zip(scores_all, indicies_all, label_ids_all):
        c = 1
        for si, ii in zip(s, i):
            if l[0] == ii:
                gt_scores.append({'gt':l[0], 'score':si, 'rank':c})
                break
            c+=1
...
def _run_biencoder(biencoder, dataloader, candidate_encoding, top_k=100, indexer=None):
    ...
    for batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            if indexer is not None:
                ...
                gt_scores.append(get_gt_score(scores, label_ids))

                scores, indicies = scores.topk(top_k) # cand id 
    ...
    return labels, nns, all_scores, gt_scores
...
def run(
    ...
    while not stopping_condition:
        ...
        top_k = args.top_k
        labels, nns, scores, gt_scores = _run_biencoder(
        ...
        if args.interactive:
            ...
            if not keep_all:
                ...
                for entity_list, score, gs in zip(nns, scores, gt_scores):
                    sample_prediction = []
                    for e_id, s in zip(entity_list, score):
                        e_title = id2title[e_id]
                        sample_prediction.append({'id':e_id, 'title':e_title, 'score':s})

                bienc_cand = [i.tolist() for i in nns]
                bienc_scores = [i.tolist() for i in scores]
                bi_pred = {'gt_score':gt_scores, 'bienc_cand':bienc_cand, 'bienc_scores':bienc_scores}
        ...
        if args.interactive:
            ...
            for entity_list, index_list, scores_list in zip(
                ...
                for index in index_list:
                    ...
                    e_title = id2title[e_id]
                    pred_ids.append(e_id)
                    sample_prediction.append(e_title)
                    sample_scores.append(scores_list[index])
                cross_pred_ids.append(pred_ids)
                predictions.append(sample_prediction)
                scores.append(sample_scores)

            cross_pred = {'titles':predictions, 'crossenc_cand':cross_pred_ids, 'crossenc_scores':scores}
